...
static const char* const PHRASES_HEAP_CAUTION[] = {
    ...
    "heap doing yoga. badly.",
    "free list looking thin",
    "pig counts kilobytes now"
};
static const int PHRASES_HEAP_CAUTION_COUNT =
    (int)(sizeof(PHRASES_HEAP_CAUTION) / sizeof(PHRASES_HEAP_CAUTION[0]));
...
static const char* const PHRASES_HEAP_WARNING[] = {
    ...
    "largest block is a joke",
    "new() laughs in my face",
    "heap on a diet again"
};
static const int PHRASES_HEAP_WARNING_COUNT =
    (int)(sizeof(PHRASES_HEAP_WARNING) / sizeof(PHRASES_HEAP_WARNING[0]));
...
static const char* const PHRASES_HEAP_CRITICAL[] = {
    ...
    "HEAP FLATLINED BRUV",
    "OOM. PIG SEES LIGHT.",
    "STACK WHISPERS SORRY"
};
static const int PHRASES_HEAP_CRITICAL_COUNT =
    (int)(sizeof(PHRASES_HEAP_CRITICAL) / sizeof(PHRASES_HEAP_CRITICAL[0]));

static const char* const PHRASES_HEAP_RECOVERY[] = {
    "TLSF coalesced. pig lives.",
    "free blocks returned. praise.",
    "defrag worked. praise.",
    "heap breathed. pig exhaled.",
    "kilobytes reunited. cute."
};
static const int PHRASES_HEAP_RECOVERY_COUNT =
    (int)(sizeof(PHRASES_HEAP_RECOVERY) / sizeof(PHRASES_HEAP_RECOVERY[0]));
...
static const char* const PHRASES_TIME_SPECIAL[] = {
    ...
    "00:00. pig persists.",
    "11:11. make a wish. deauth.",
    "09:00. corporate RF begins."
};
...
static const char* const PHRASES_DENSITY_HIGH[] = {
    ...
    "SSID soup tonight",
    "beacons like confetti",
    "too many truffles. crisis."
};
static const int PHRASES_DENSITY_HIGH_COUNT =
    (int)(sizeof(PHRASES_DENSITY_HIGH) / sizeof(PHRASES_DENSITY_HIGH[0]));
...
static const char* const PHRASES_DENSITY_LOW[] = {
    ...
    "spectrum on silent mode",
    "wifi went on holiday",
    "desert channel. no oases."
};
static const int PHRASES_DENSITY_LOW_COUNT =
    (int)(sizeof(PHRASES_DENSITY_LOW) / sizeof(PHRASES_DENSITY_LOW[0]));

static const char* const PHRASES_DENSITY_TRANSITION[] = {
    "APs vanishing fast",
    "truffles appearing innit",
    "landscape changed. pig notices.",
    "RF weather shifted. interesting.",
    "new crowd. pig recalibrates."
};
static const int PHRASES_DENSITY_TRANSITION_COUNT =
    (int)(sizeof(PHRASES_DENSITY_TRANSITION) / sizeof(PHRASES_DENSITY_TRANSITION[0]));
...
static const char* const PHRASES_CHALLENGE_CLOSE[] = {
    ...
    "one more push. glory adjacent.",
    "almost. pig holding breath."
};
static const int PHRASES_CHALLENGE_CLOSE_COUNT =
    (int)(sizeof(PHRASES_CHALLENGE_CLOSE) / sizeof(PHRASES_CHALLENGE_CLOSE[0]));
...
static const char* const PHRASES_RETURN_NORMAL[] = {
    ...
    "same pig. new excuses."
};
static const char* const PHRASES_RETURN_LONG[] = {
    "gone ages. pig coped. barely.",
    "pig was lonely. pig lies.",
    "long absence. heap survived.",
    "dust on the snout. wipe it.",
    "calendar called. pig answered."
};

// One-shot patch notes in pig voice (NVS news_v). Bump version when shipping more.
static const uint8_t FEATURE_NEWS_VER = 2;
static const char* const PHRASES_FEATURE_NEWS[] = {
    "patch notes? pig snorts.",
    "ADDED files + pigpass resume",
    "ADDED mask gen, live try",
    "FIX evilpig + blues no die"
};
static const uint8_t PHRASES_FEATURE_NEWS_COUNT =
    (uint8_t)(sizeof(PHRASES_FEATURE_NEWS) / sizeof(PHRASES_FEATURE_NEWS[0]));
...
const char* PHRASES_DYNAMIC[] = {
    ...
    "$NET APs. snout overtime.",
    "lvl $LVL. still oinking.",
    "$HS hashes. pig proud-ish.",
    "$DEAUTH frames. chaos tax.",
    "$KM walked. truffles pending.",
    "stats check: $NET / $HS. spicy.",
    "$NAME, $HS is a vibe."
};
...
const char* PHRASES_RARE_LORE[] = {
    ...
    "readme lied. pig fixed.",
    "undefined behavior: personality",
    "TODO: become soup. cancelled.",
    "heap ghosts in the barn",
    "this build knows too much",
    "comment said temporary. 2024."
};
...
const char* PHRASES_HAPPY_CD[] = {
    ...
    "horse find di way",
    "jah guide di snout",
    "one love one channel",
    "heart full of beacons",
    "irie mud underfoot",
    "snout smile real wide",
    "blessings in di air"
};
...
const char* PHRASES_HAPPY_WARHOG[] = {
    ...
    "objective achieved",
    "morale high. keep pushin",
    "unit cohesion excellent",
    "good order and discipline",
    "chow hall mood: peak",
    "sitrep: hog is smiling"
};
...
const char* PHRASES_HUNTING[] = {
    ...
    "mud radar online",
    "stalking the SSIDs",
    "quiet trot. loud intent.",
    "truffle or bust mate"
};
...
const char* PHRASES_OINK_QUIET[] = {
    ...
    "802.11 wasteland",
    "beacons on strike",
    "pig paid for premium air",
    "silence is not golden",
    "even WiFi ghosted me"
};
...
const char* PHRASES_SLEEPY_CD[] = {
    ...
    "easy does it",
    "soft rain. soft wait.",
    "snout closed. heart open",
    "breathe. listen. later.",
...
const char* PHRASES_SLEEPY_WARHOG[] = {
    ...
    "maintaining watch",
    "idle but ready",
    "eyes open. boots still.",
    "overwatch continues",
    "coffee lukewarm. still in",
    "quiet is a weapon too"
};
...
const char* PHRASES_BORED[] = {
    ...
    "where horse at",
    "barn too quiet",
    "watching paint dry. RF.",
    "loading more nothing...",
    "snout filed a complaint",
    "is this a Faraday barn?",
    "bring me drama. or SSIDs."
};
...
const char* PHRASES_WARHOG[] = {
    ...
    "mapping the RF terrain",
    "keep eyes on the flanks",
    "log it. move. log again.",
    "recon never clocks out"
};
...
const char* PHRASES_WARHOG_FOUND[] = {
    ...
    "position marked sir",
    "new contact on the map",
    "file it under interesting",
    "good find. keep sweeping."
};
...
static const int PHRASES_BIRD_KILL_COUNT =
    (int)(sizeof(PHRASES_BIRD_KILL) / sizeof(PHRASES_BIRD_KILL[0]));
...
const char* PHRASES_PIGGYBLUES_TARGETED[] = {
    ...
    "%s clocked at %ddB hunty",
    "werk that %s signal %ddB",
    "%s baby girl pop off %ddB"
};
...
const char* PHRASES_PIGGYBLUES_STATUS[] = {
    ...
    "runway: %d/%d still walking",
    "%d looks served. %d watching."
};
...
const char* PHRASES_PIGGYBLUES_IDLE[] = {
    ...
    "lip sync for ur BLE life",
    "untucked and scanning",
    "the library is open. BLE edition.",
    "shade thrown at 2.4GHz"
};
...
const char* PHRASES_PMKID_OINK[] = {
    ...
    "ghost hash in the bag",
    "no deauth. pure skill."
};
...
const char* PHRASES_PMKID_CD[] = {
    ...
    "pmkid inna air ya",
    "peaceful pwn irie",
    "soft power. hard hash.",
    "listen long. win quiet."
};
...
const char* PHRASES_RARE[] = {
    ...
    "this little piggy went pwn",
    "be the beacon you sniff",
    "ASCII pig supremacy",
    "snort first. ask later.",
    "RF is just spicy mud",
    "deauth is a love language",
    "IPv6? pig has anxiety",
    "WPA2 called. wants respect.",
    "open portal. closed dignity.",
    "handshake or handwave?",
    "beacon interval too clingy",
    "channel hop like a drama queen",
    "promiscuous mode. no shame.",
    "softAP with hard feelings",
    "captive portal. free will sold.",
    "mask attack. fashion optional.",
    "wordlist longer than patience",
    "resume from 444. pig remembers.",
    "FILES menu. dig or delete.",
    "0 for snap. P for pain.",
    "YOU DIED was so last build",
    "clean exit. dirty spectrum.",
    "heap health > mental health",
    "largest free block is a mood",
    "fragmentation is a lifestyle",
    "brew the heap. serve chaos.",
    "SSID: [HIDDEN] personality: not",
    "client joined. plot thickens.",
    "loot on SD. ego in SPIFFS.",
    "lab only. conscience optional.",
    "authorized network. allegedly.",
    "snout-certified nonsense",
    "oink.exe has stopped caring",
    "more jokes. same pig. deal."
};
...
void Mood::init() {
    ...
    // Phase 10: Load saved mood + feature-news flag from NVS
    ...
    uint8_t newsSeen = moodPrefs.getUChar("news_v", 0);
    moodPrefs.end();
    ...
    if (savedTime > 0) {
        ...
        if (sessions > 0 && (sessions % 25) == 0) {
            ...
            const int n = (int)(sizeof(PHRASES_RETURN_QUICK) / sizeof(PHRASES_RETURN_QUICK[0]));
            SET_PHRASE(currentPhrase, PHRASES_RETURN_QUICK[random(0, n)]);
        } else if (savedMood > 60) {
            SET_PHRASE(currentPhrase, "missed me piggy?");
        } else if (savedMood < -20) {
            SET_PHRASE(currentPhrase, "back for more..");
        } else {
            const int n = (int)(sizeof(PHRASES_RETURN_NORMAL) / sizeof(PHRASES_RETURN_NORMAL[0]));
            SET_PHRASE(currentPhrase, PHRASES_RETURN_NORMAL[random(0, n)]);
        }
    } else {
        happiness = 50;
    }
    ...
    // One-shot feature news in dialogue (same snout humour). Queue overwrites return line after a beat.
    if (newsSeen < FEATURE_NEWS_VER && PHRASES_FEATURE_NEWS_COUNT > 0) {
        phraseQueueCount = 0;
        // Cap to queue capacity (4)
        uint8_t n = PHRASES_FEATURE_NEWS_COUNT;
        if (n > 4) n = 4;
        for (uint8_t i = 0; i < n; i++) {
            queuePhrase(PHRASES_FEATURE_NEWS[i]);
        }
        // Show first line ASAP
        lastQueuePop = millis() - PHRASE_CHAIN_DELAY_MS;
        SET_PHRASE(currentPhrase, PHRASES_FEATURE_NEWS[0]);
        // Drop first from queue so processQueue continues with rest
        if (phraseQueueCount > 0) {
            phraseQueueCount--;
            if (phraseQueueCount > 0) {
                memmove(phraseQueue[0], phraseQueue[1],
                        phraseQueueCount * sizeof(phraseQueue[0]));
            }
            phraseQueue[phraseQueueCount][0] = '\0';
        }
        moodPrefs.begin(MOOD_NVS_NAMESPACE, false);
        moodPrefs.putUChar("news_v", FEATURE_NEWS_VER);
        moodPrefs.end();
    }
}
...
bool Mood::pickTimePhraseIfDue(uint32_t now) {
    ...
    if (hour >= 5 && hour < 8) {
        if (isWarhog) {
            const int n = (int)(sizeof(PHRASES_TIME_EARLY_WARHOG) / sizeof(PHRASES_TIME_EARLY_WARHOG[0]));
            SET_PHRASE(currentPhrase, PHRASES_TIME_EARLY_WARHOG[random(0, n)]);
        } else {
            const int n = (int)(sizeof(PHRASES_TIME_EARLY_OINK) / sizeof(PHRASES_TIME_EARLY_OINK[0]));
            SET_PHRASE(currentPhrase, PHRASES_TIME_EARLY_OINK[random(0, n)]);
        }
        return true;
    }
    ...
    if (hour >= 1 && hour < 4) {
        if (isCD) {
            const int n = (int)(sizeof(PHRASES_TIME_LATENIGHT_CD) / sizeof(PHRASES_TIME_LATENIGHT_CD[0]));
            SET_PHRASE(currentPhrase, PHRASES_TIME_LATENIGHT_CD[random(0, n)]);
        } else if (isWarhog) {
            const int n = (int)(sizeof(PHRASES_TIME_LATENIGHT_WARHOG) / sizeof(PHRASES_TIME_LATENIGHT_WARHOG[0]));
            SET_PHRASE(currentPhrase, PHRASES_TIME_LATENIGHT_WARHOG[random(0, n)]);
        } else {
            const int n = (int)(sizeof(PHRASES_TIME_LATENIGHT_OINK) / sizeof(PHRASES_TIME_LATENIGHT_OINK[0]));
            SET_PHRASE(currentPhrase, PHRASES_TIME_LATENIGHT_OINK[random(0, n)]);
        }
        return true;
    }
...
void Mood::updateAvatarState() {
    ...
    // Animation lab (SETTINGS → ANIM TEST): leave face/FX alone while IDLE
    // so -/= demos stick until the user steps to the next one.
    PorkchopMode mode = porkchop.getMode();
    if (mode == PorkchopMode::IDLE && Config::personality().animTest) {
        return;
    }
    // Fruit Run drives its own expressions (jump happy / oink-over sad)
    if (mode == PorkchopMode::FRUIT_RUN_MODE) {
        return;
    }
    ...
    switch (mode) {
        ...
        default:
            // IDLE / menus: keep eyes open more often (SLEEPY only when very low)
            // Fixes "eyes always closed while standing still"
            if (mode == PorkchopMode::IDLE) {
                if (effectiveMood > MOOD_PEEK_HIGH_THRESHOLD) {
                    Avatar::setState(AvatarState::EXCITED);
                } else if (effectiveMood > 20) {
                    Avatar::setState(AvatarState::HAPPY);
                } else if (effectiveMood > -40) {
                    Avatar::setState(AvatarState::NEUTRAL);  // open eyes
                } else if (effectiveMood > -75) {
                    Avatar::setState(AvatarState::SLEEPY);   // half-lidded
                } else {
                    Avatar::setState(AvatarState::SAD);
                }
            } else {
...
const char* PHRASES_MENU_IDLE[] = {
    ...
    "promiscuous mode",
    "sudo make bacon",
    "SYSTEM -> FILES digs SD",
    "menu stares back",
    "choose violence wisely",
    "idle is a skill issue",
    "pig waiting. dramatic."
};
