...
bool Config::loadPersonality() {
    ...
    if (!doc["soundLevel"].isNull()) {
    personalityConfig.soundLevel = doc["soundLevel"] | 1;
    if (personalityConfig.soundLevel > 5) {
        personalityConfig.soundLevel = 5;
    }
} else {
    ...
    // skyMode replaces color themes (legacy themeIndex ignored)
    uint8_t sky = doc["skyMode"] | 0;
    if (sky >= SKY_MODE_COUNT) sky = 0;
    personalityConfig.skyMode = sky;
    ...
    if (!doc["seasonMode"].isNull()) {
        ...
        if (sm >= SEASON_MODE_COUNT) sm = 0;
        personalityConfig.seasonMode = sm;
    } else if (!doc["grassStyle"].isNull()) {
        uint8_t grass = doc["grassStyle"] | 0;
        // old: 0 CLASSIC→SUMMER, 1 SPRING, 2 AUTUMN, 3 WINTER
        if (grass == 0) personalityConfig.seasonMode = (uint8_t)SeasonMode::SUMMER;
        else if (grass == 1) personalityConfig.seasonMode = (uint8_t)SeasonMode::SPRING;
        else if (grass == 2) personalityConfig.seasonMode = (uint8_t)SeasonMode::AUTUMN;
        else if (grass == 3) personalityConfig.seasonMode = (uint8_t)SeasonMode::WINTER;
        else personalityConfig.seasonMode = (uint8_t)SeasonMode::AUTO;
    } else {
        personalityConfig.seasonMode = (uint8_t)SeasonMode::AUTO;
    }
    personalityConfig.animTest = doc["animTest"] | false;
    personalityConfig.wolfEnabled = doc["wolfEnabled"] | true;
    {
        // Prefer scrollSpeed; fall back to old walkSpeed key if present
        int ss = 6;
        if (!doc["scrollSpeed"].isNull()) ss = doc["scrollSpeed"] | 6;
        else if (!doc["walkSpeed"].isNull()) ss = doc["walkSpeed"] | 6;
        if (ss < 1) ss = 1;
        if (ss > 10) ss = 10;
        personalityConfig.scrollSpeed = (uint8_t)ss;
    }
    personalityConfig.fruitTreesAmbient = doc["fruitTreesAmbient"] | true;
    ...
    Serial.printf("[CONFIG] Personality: %s (mood: %d, sound: %d, bright: %d%%, dim: %ds, sky: %d)\n",
...
void Config::savePersonalityToSPIFFS() {
    ...
    doc["scrollSpeed"] = personalityConfig.scrollSpeed;
    doc["fruitTreesAmbient"] = personalityConfig.fruitTreesAmbient;
