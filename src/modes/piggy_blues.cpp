...
// Exit used to random-reboot ("YOU DIED" roulette). That was intentional joke
// but kills session / WiFi recon — always clean-exit now.
static const uint16_t EXIT_XP_BONUS = 15;
...
void PiggyBluesMode::start() {
    ...
    // Reset state for new session — no blocking warning modal
    init();
...
void PiggyBluesMode::stop() {
    ...
    // Clean exit only — no random reboot / "YOU DIED"
    Display::showToast("BLUES OFF  +15 XP");
