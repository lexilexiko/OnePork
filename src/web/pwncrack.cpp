...
PwncrackSyncResult Pwncrack::syncCaptures(PwncrackProgressCallback cb) {
    ...
    // Park NetworkRecon — promiscuous callback может фрагментировать heap во время HTTP
    bool wasReconRunning = NetworkRecon::isRunning();
    if (wasReconRunning) {
        Serial.println("[PWNCRACK] Pausing NetworkRecon for sync");
        NetworkRecon::pause();
        NetworkRecon::freeNetworks();   // ~19KB reclaim
    }

    // Park TracksMenu (Wigle) если открыт — ест heap на списке/UI state.
    // Не восстанавливаем после sync: пользователь сам переоткроет.
    if (TracksMenu::isActive()) {
        Serial.println("[PWNCRACK] Hiding TracksMenu for sync");
        TracksMenu::hide();
    }
    ...
    // Restore NetworkRecon если был запущен
    if (wasReconRunning) {
        Serial.println("[PWNCRACK] Resuming NetworkRecon after sync");
        NetworkRecon::resume();
    }
