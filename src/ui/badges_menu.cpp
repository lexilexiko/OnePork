...
void BadgesMenu::draw(M5Canvas& canvas) {
    ...
    for (uint8_t i = scrollOffset; i < TOTAL_ACHIEVEMENTS && i < scrollOffset + VISIBLE_ITEMS; i++) {
        ...
        uint16_t accent = hasIt ? UiStyle::GOLD : UiStyle::PURPLE;
        uiListRow(canvas, y, lineHeight, sel, accent);
        canvas.setTextColor(sel ? UiStyle::BG : (hasIt ? UiStyle::TEXT : UiStyle::DIM));
...
void BadgesMenu::drawDetail(M5Canvas& canvas) {
    ...
    // Gold panel modal
    canvas.fillRoundRect(boxX - 2, boxY - 2, boxW + 4, boxH + 4, 8, UiStyle::TITLE);
    canvas.fillRoundRect(boxX, boxY, boxW, boxH, 8, UiStyle::PANEL);
    
    canvas.setTextColor(UiStyle::TITLE, UiStyle::PANEL);
    canvas.setTextSize(1);
