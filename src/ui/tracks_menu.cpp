...
// Same soft gate as Hashes — Critical pressure is for TLS, not dir listing
...
void TracksMenu::scanFiles() {
    ...
    if (!heapOkForScan()) {
        Serial.println("[TRACKS] Freeing NetworkRecon list to make room for scan");
        NetworkRecon::freeNetworks();
    }

    if (!heapOkForScan()) {
        Serial.printf("[TRACKS] Scan deferred: free=%u largest=%u (need %u/%u)\n",
...
void TracksMenu::handleInput() {
    ...
    if (scanDeferredHeap && files.empty() &&
        (M5Cardputer.Keyboard.isKeyPressed('r') || M5Cardputer.Keyboard.isKeyPressed('R'))) {
        Serial.println("[TRACKS] Manual scan retry (R)");
        scanFiles();
        return;
    }
...
void TracksMenu::update() {
    ...
    if (!syncModalActive) {
        ...
        // Auto-retry deferred scan when heap recovers
        if (scanDeferredHeap && files.empty() && !scanInProgress) {
            ...
            if (now - lastDeferRetryMs > 2000) {
                ...
                if (freeH >= kScanMinFree && largest >= kScanMinContig) {
                    Serial.println("[TRACKS] Heap recovered — retrying deferred scan");
                    scanFiles();
                }
            }
        }
...
void TracksMenu::draw(M5Canvas& canvas) {
    ...
    for (uint8_t i = scrollOffset; i < files.size() && i < scrollOffset + VISIBLE_ITEMS; i++) {
        ...
        uiListRow(canvas, y, lineHeight, sel, UiStyle::GREEN);
