...
// Directory listing is cheap — do NOT gate on HeapPressureLevel::Critical.
// Critical often means "fragmented for TLS" (free 40KB but largest block 8KB),
// while SD openNextFile only needs a few KB. OINK "fixed" this by brewing heap.
static constexpr size_t kScanMinFree   = 12000;
static constexpr size_t kScanMinContig = 4096;
...
bool HashesMenu::scanCaptures() {
    ...
    // Drop WPA-SEC caches first (can free several KB before we even look at heap)
    WPASec::freeCacheMemory();

    auto heapOkForScan = []() {
        size_t freeH = ESP.getFreeHeap();
        size_t largest = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
        return freeH >= kScanMinFree && largest >= kScanMinContig;
    };

    if (!heapOkForScan()) {
        // Recon network list holds ~19KB reserved capacity — free for the scan.
        // Recon will re-discover APs; listing .pcap does not need that vector.
        Serial.println("[HASHES] Freeing NetworkRecon list to make room for scan");
        NetworkRecon::freeNetworks();
    }

    if (!heapOkForScan()) {
        Serial.printf("[HASHES] Scan deferred: free=%u largest=%u (need %u/%u)\n",
                      (unsigned)ESP.getFreeHeap(),
                      (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_8BIT),
                      (unsigned)kScanMinFree, (unsigned)kScanMinContig);
...
void HashesMenu::update() {
    ...
    if (!syncModalActive) {
        ...
        // Auto-retry deferred scan when heap recovers (no need to leave menu)
        if (scanDeferredHeap && captures.empty() && !scanInProgress) {
            static uint32_t lastDeferRetryMs = 0;
            uint32_t now = millis();
            if (now - lastDeferRetryMs > 2000) {
                lastDeferRetryMs = now;
                size_t freeH = ESP.getFreeHeap();
                size_t largest = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
                if (freeH >= kScanMinFree && largest >= kScanMinContig) {
                    Serial.println("[HASHES] Heap recovered — retrying deferred scan");
                    scanCaptures();
                }
            }
        }
...
void HashesMenu::handleInput() {
    ...
    // Manual retry when scan was deferred for heap
    if (scanDeferredHeap && captures.empty() &&
        (M5Cardputer.Keyboard.isKeyPressed('r') || M5Cardputer.Keyboard.isKeyPressed('R'))) {
        Serial.println("[HASHES] Manual scan retry (R)");
        scanCaptures();
        return;
    }
...
void HashesMenu::draw(M5Canvas& canvas) {
    ...
    if (captures.empty()) {
        ...
        if (scanDeferredHeap) {
            ...
            canvas.print("LOW CONTIGUOUS HEAP");
            canvas.setCursor(4, 68);
            canvas.print("R=RETRY  OR RUN OINK");
            canvas.setCursor(4, 84);
            canvas.print("(OINK COALESCES HEAP)");
        } else {
            canvas.setCursor(4, 36);
