...
void BoarBrosMenu::draw(M5Canvas& canvas) {
    ...
    for (uint8_t i = scrollOffset; i < bros.size() && i < scrollOffset + VISIBLE_ITEMS; i++) {
        ...
        uiListRow(canvas, y, lineHeight, sel, UiStyle::CYAN);
        ...
        // Full BSSID
        canvas.setTextColor(sel ? UiStyle::BG : UiStyle::GOLD);
...
void BoarBrosMenu::drawDeleteConfirm(M5Canvas& canvas) {
    ...
    canvas.fillRoundRect(boxX - 2, boxY - 2, boxW + 4, boxH + 4, 8, UiStyle::RED);
