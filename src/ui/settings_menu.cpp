...
static const RootEntry kRootEntries[] = {
    ...
    {"NETWORK", "WIFI CREDENTIALS", true, GROUP_NET, SET_SKY},
    {"INTEGRATION", "API KEYS", true, GROUP_INTEG, SET_SKY},
    {"RADIO", "WIFI SCAN/ATTACK TIMING", true, GROUP_RADIO, SET_SKY},
    {"GPS", "GPS MODULE SETTINGS", true, GROUP_GPS, SET_SKY},
    {"BLE", "BLE ATTACK TUNING", true, GROUP_BLE, SET_SKY},
    {"JANUS HOG", "ESP32-C5 5GHZ BOARD", true, GROUP_C5, SET_SKY}
};

static const char* const kSkyModeLabels[SKY_MODE_COUNT] = {
    "AUTO",
    "DAY",
    "NIGHT"
};
...
static const char* const kPigSkinLabels[PIG_SKIN_COUNT] = {
    "CLASSIC",
    "BLUSH",
    "HOG",
    "ZOMBIE",
...
static bool isPersonalitySetting(SettingId id) {
    return id == SET_SKY || id == SET_BRIGHTNESS || id == SET_SOUND ||
           id == SET_DIM_AFTER || id == SET_DIM_LEVEL || id == SET_G0_ACTION ||
           id == SET_BOOT_MODE || id == SET_CALLSIGN || id == SET_PIG_SKIN ||
           id == SET_SEASON || id == SET_ANIM_TEST || id == SET_WOLF ||
           id == SET_SCROLL_SPD || id == SET_FRUIT_TREES;
}

static const char* getSkyModeLabel(int idx) {
    if (idx < 0 || idx >= (int)SKY_MODE_COUNT) return kSkyModeLabels[0];
    return kSkyModeLabels[idx];
}
...
static const char* getSeasonModeLabel(int idx) {
    if (idx < 0 || idx >= (int)SEASON_MODE_COUNT) return kSeasonModeLabels[0];
    return kSeasonModeLabels[idx];
}
...
static const EntryData* getGroupEntries(GroupId group, size_t* count) {
    switch (group) {
        case GROUP_SCENE:
            *count = sizeof(kSceneEntries) / sizeof(kSceneEntries[0]);
            return kSceneEntries;
        case GROUP_NET:
...
static int getSettingValue(SettingId id) {
    switch (id) {
        case SET_SKY:
            return Config::personality().skyMode;
        case SET_BRIGHTNESS:
        ...
        case SET_PIG_SKIN:
            return Config::personality().pigSkin;
        case SET_SEASON:
            return Config::personality().seasonMode;
        case SET_ANIM_TEST:
...
static bool setSettingValue(SettingId id, int value) {
    switch (id) {
        case SET_SKY: {
            ...
            if (newVal >= SKY_MODE_COUNT) newVal = 0;
            if (Config::personality().skyMode == newVal) return false;
            Config::personality().skyMode = newVal;
            return true;
        }
        case SET_PIG_SKIN: {
            uint8_t newVal = static_cast<uint8_t>(value);
        ...
        case SET_SEASON: {
            uint8_t newVal = static_cast<uint8_t>(value);
            if (newVal >= SEASON_MODE_COUNT) newVal = 0;
            if (Config::personality().seasonMode == newVal) return false;
            Config::personality().seasonMode = newVal;
            // Toast current active season name for feedback
        ...
        case SET_ANIM_TEST: {
            bool on = value != 0;
            if (Config::personality().animTest == on) return false;
            Config::personality().animTest = on;
            if (on) {
                Display::showToast("ANIM TEST: -/= ON IDLE", 1800);
            }
            return true;
        }
        case SET_WOLF: {
        ...
        case SET_SCROLL_SPD: {
            ...
            if (v < 1) v = 1;
            if (v > 10) v = 10;
            if (Config::personality().scrollSpeed == (uint8_t)v) return false;
            Config::personality().scrollSpeed = (uint8_t)v;
            char buf[24];
            snprintf(buf, sizeof(buf), "SCROLL SPD: %d/10", v);
            Display::showToast(buf, 1000);
            return true;
        }
        case SET_FRUIT_TREES: {
            bool on = value != 0;
...
void SettingsMenu::draw(M5Canvas& canvas) {
    // Colorful farm UI (matches main menu)
    uiListBackground(canvas);
    canvas.setTextSize(2);

    const int lineHeight = 18;
    static const uint16_t ROW_ACCENT[] = {
        UiStyle::CYAN, UiStyle::GOLD, UiStyle::PINK, UiStyle::GREEN,
        UiStyle::PURPLE, UiStyle::RED, UiStyle::GOLD, UiStyle::CYAN
    };
    GroupId group = static_cast<GroupId>(activeGroup);
    ...
    if (group == GROUP_NONE) {
        ...
        for (uint8_t i = 0; i < VISIBLE_ROOT_ITEMS && (rootScroll + i) < rootCount; i++) {
            ...
            uint16_t accent = ROW_ACCENT[idx % 8];

            uiListRow(canvas, y, lineHeight, selected, accent);
            canvas.setTextColor(selected ? UiStyle::BG : UiStyle::TEXT);

            canvas.setTextDatum(top_left);
            canvas.drawString(entry.label, 10, y + 2);
            ...
            if (entry.isGroup) {
                ...
                if (direct) {
                    ...
                    if (direct->type == SettingType::TOGGLE) {
                        ...
                        valBuf[sizeof(valBuf) - 1] = '\0';
                    } else if (direct->id == SET_SKY) {
                        const char* skyLabel = getSkyModeLabel(value);
                        if (selected && editing) {
                            snprintf(valBuf, sizeof(valBuf), "[%s]", skyLabel);
                        } else {
                            strncpy(valBuf, skyLabel, sizeof(valBuf) - 1);
                            valBuf[sizeof(valBuf) - 1] = '\0';
                        }
                    ...
                    } else if (direct->id == SET_PIG_SKIN) {
                        const char* skinLabel = getPigSkinLabel(value);
                        if (selected && editing) {
                            snprintf(valBuf, sizeof(valBuf), "[%s]", skinLabel);
                        } else {
                            strncpy(valBuf, skinLabel, sizeof(valBuf) - 1);
                            valBuf[sizeof(valBuf) - 1] = '\0';
                        }
                    } else if (direct->id == SET_SEASON) {
                        const char* seasonLabel = getSeasonModeLabel(value);
                        if (selected && editing) {
                            snprintf(valBuf, sizeof(valBuf), "[%s]", seasonLabel);
                        } else {
                            strncpy(valBuf, seasonLabel, sizeof(valBuf) - 1);
                            valBuf[sizeof(valBuf) - 1] = '\0';
                        }
            ...
            if (valBuf[0] != '\0') {
                ...
                canvas.setTextColor(selected ? UiStyle::BG : UiStyle::GOLD);
                canvas.drawString(valBuf, DISPLAY_W - 8, y + 2);
            }
        ...
        if (rootScroll + VISIBLE_ROOT_ITEMS < rootCount) {
            ...
            canvas.drawString("v", DISPLAY_W / 2, MAIN_H - 12);
        }
        return;
    }
    ...
    // Group header
    canvas.fillRect(0, 0, DISPLAY_W, lineHeight, UiStyle::PANEL);
    canvas.fillRect(0, 0, 3, lineHeight, UiStyle::TITLE);
    canvas.setTextColor(UiStyle::TITLE);
    canvas.setTextDatum(top_left);
    canvas.drawString(getGroupLabel(group), 8, 2);
    ...
    for (uint8_t i = 0; i < VISIBLE_GROUP_ITEMS && (groupScroll + i) < count; i++) {
        ...
        if (entry.type == SettingType::ACTION) {
            ...
            } else if (entry.id == SET_SKY) {
                const char* skyLabel = getSkyModeLabel(value);
                if (selected && editing) snprintf(valBuf, sizeof(valBuf), "[%s]", skyLabel);
                else { strncpy(valBuf, skyLabel, sizeof(valBuf) - 1); valBuf[sizeof(valBuf) - 1] = '\0'; }
            } else if (entry.id == SET_SEASON) {
                const char* seasonLabel = getSeasonModeLabel(value);
                if (selected && editing) snprintf(valBuf, sizeof(valBuf), "[%s]", seasonLabel);
                else { strncpy(valBuf, seasonLabel, sizeof(valBuf) - 1); valBuf[sizeof(valBuf) - 1] = '\0'; }
            } else if (entry.id == SET_PIG_SKIN) {
                const char* skinLabel = getPigSkinLabel(value);
