...
void uiListRow(M5Canvas& canvas, int y, int lineH, bool selected, uint16_t accent) {
    if (selected) {
        ...
        canvas.fillRect(4, y, DISPLAY_W - 8, lineH - 2, UiStyle::PANEL);
        canvas.fillRect(4, y, 3, lineH - 2, accent);
    }
}
...
void Display::drawTopBar() {
    ...
    // Match sky when avatar scene is active (2.5D continuous look)
    PorkchopMode mode = porkchop.getMode();
    const bool avatarScene =
    ...
    switch (mode) {
        ...
        case PorkchopMode::COREDUMP:
            ...
            break;
        case PorkchopMode::FILES_MODE:
            snprintf(modeBuf, sizeof(modeBuf), "FILES");
            modeColor = barFg;
            break;
        case PorkchopMode::DIAGDATA:
        ...
        case PorkchopMode::BACON_MODE:
            ...
            break;
        case PorkchopMode::MICPORK_MODE:
            snprintf(modeBuf, sizeof(modeBuf), "MICPORK");
            modeColor = barFg;
            break;
        case PorkchopMode::FRUIT_RUN_MODE:
        ...
        case PorkchopMode::IR_PORK_MODE:
            ...
            break;
        case PorkchopMode::EVILPIG_MODE:
            // Short phase label — no clutter next to battery
            if (EvilPigMode::getPhase() == EvilPigMode::Phase::LOOT)
                snprintf(modeBuf, sizeof(modeBuf), "LOOT");
            else if (EvilPigMode::getPhase() == EvilPigMode::Phase::PORTAL)
                snprintf(modeBuf, sizeof(modeBuf), "PORTAL");
            else
                snprintf(modeBuf, sizeof(modeBuf), "EVIL");
            modeColor = barFg;
            break;
        case PorkchopMode::JANUS_HOG_MODE:
        {
            ...
            modeColor = barFg;  // All COLOR_* aliases resolve to fg
            break;
        }
        case PorkchopMode::SD_FORMAT:
        ...
        case PorkchopMode::CHARGING:
            ...
            break;
        case PorkchopMode::PIGPASS_MODE:
            snprintf(modeBuf, sizeof(modeBuf), "PIGPASS");
            modeColor = barFg;
            break;
    }
    ...
    // Right side: battery + status icons (same ink as left on sky bar)
    topBar.setTextColor(barFg);
    topBar.setTextDatum(top_right);
...
void Display::drawBottomBar() {
    // Terraria-style dirt ground strip (HP/stats zone)
    // Top fringe follows season so winter is not a permanent green stripe
    ...
    switch (season) {
        ...
        case Season::SUMMER:
            ...
            break;
        case Season::AUTUMN:
            fringeTop = 0x8AC0;  // gold-brown
            DIRT_MID  = 0x7A20;
            DIRT_DARK = 0x5120;
            break;
        case Season::WINTER:
            fringeTop = 0xDEFB;  // frost / snow lip
            DIRT_MID  = 0x6B6D;  // frozen dirt
            DIRT_DARK = 0x4208;
            DIRT_LITE = 0xC618;
            break;
        case Season::RETRO:
    ...
    // Dirt fill + seasonal fringe on top edge (meets scene ground / snow banks)
    bottomBar.fillSprite(DIRT_MID);
    bottomBar.fillRect(0, 0, DISPLAY_W, 2, fringeTop);
    if (season == Season::WINTER) {
        // Soft snow dust on the lip
        for (int x = 0; x < DISPLAY_W; x += 3) {
            uint32_t h = (uint32_t)x * 1103515245u;
            if ((h & 3) != 0) bottomBar.drawPixel(x, 0, 0xFFFF);
            if ((h & 7) == 0) bottomBar.drawPixel(x + 1, 1, 0xC618);
        }
    ...
    // Speckle pebbles
    for (int x = 0; x < DISPLAY_W; x += 5) {
        uint32_t h = (uint32_t)x * 1103515245u + 12345u;
        if ((h & 5) == 0) {
            bottomBar.drawPixel(x, 4 + (h % 6), DIRT_LITE);
        }
        if ((h & 9) == 0) {
            bottomBar.drawPixel(x + 2, 3 + (h % 5), DIRT_DARK);
        }
    }
    ...
    if (mode == PorkchopMode::WARHOG_MODE) {
        ...
        statsStr = statsBuf;
    } else if (mode == PorkchopMode::FILES_MODE) {
        FilesMenu::getStatusLine(statsBuf, sizeof(statsBuf));
        statsStr = statsBuf;
        ...
        if (ph == EvilPigMode::Phase::LOOT) {
            statsStr = ";/.scroll V/ENT:back";
        } else if (ph == EvilPigMode::Phase::PORTAL) {
            char buf[64];
            snprintf(buf, sizeof(buf), "K:kick V:loot CL:%u %s",
                     (unsigned)EvilPigMode::getClientCount(),
                     EvilPigMode::getLastEvent());
            strncpy(statsBuf, buf, sizeof(statsBuf) - 1);
            statsBuf[sizeof(statsBuf) - 1] = '\0';
            statsStr = statsBuf;
        } else {
            statsStr = ";/.list ENT:clone R:scan V:loot";
        }
        ...
        statsStr = statsBuf;
        showHealthBar = true;
    } else if (mode == PorkchopMode::PIGPASS_MODE) {
        PigpassMode::getStatusLine(statsBuf, sizeof(statsBuf));
        statsStr = statsBuf;
    ...
    // Center: Heap health bar on dirt (heart + green/red fill)
    if (showHealthBar) {
        ...
        int heartY = 3;
        // Red heart like game HP
        drawHeartIcon(bottomBar, heartX, heartY, 0xF800);

        // Bar outline cream, fill by health
        bottomBar.drawRect(barX, barY, barW, barH, TEXT_COL);
        int fillW = (barW - 2) * pct / 100;
        if (fillW > 0) {
            uint16_t fillCol = (pct > 50) ? 0x07E0 : (pct > 20) ? 0xFFE0 : 0xF800;
            bottomBar.fillRect(barX + 1, barY + 1, fillW, barH - 2, fillCol);
        }
...
void Display::showBootSplash() {
    ...
    M5.Display.drawString("v0.1.8c (1.4.6) 0N3 P0RK", DISPLAY_W / 2, DISPLAY_H / 2 + 24);
...
void Display::drawAboutScreen(M5Canvas& canvas) {
    ...
    canvas.drawString("v0.1.8c (1.4.6)", DISPLAY_W / 2, 20);
