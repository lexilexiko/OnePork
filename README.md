```
                    Volume Zero, Issue 3, Phile 1 of 1
                          ...
                          M5PORKCHOP README
                          v0.1.8c (1.6)
                   (upstream 0.1.8c · One Pork fan package 1.6 — pWnCrack)
    ...
    3. THE PIGLET (mood, avatar, weather, seasons)
    ...
    15. ONE PORK v0.1.8c (1.6) — pWnCrack; fan packaging; donate the original
...
----[ 2.4 - HOG ON SPECTRUM (RF analysis) [H]
    ...
    5GHZ ACTION PROMPT (press [ENTER] on 5GHz network):
        ...
        - [0] Packet Monitor - channel traffic (PPS from C5)
...
--[ 3 - THE PIGLET (mood, avatar, weather, seasons)
    ...
    the pig also has seasons now. grass remembers summer.
...
----[ 3.2 - AVATAR
    ...
    more animation frames than most AAA loading screens:
        ...
        - expression diversity: happy squint (^_^), soft blink arcs,
          sniff focus, excited sparkle eyes, angry brows, sad tear
        - pig skins (SETTINGS → PIG SKIN):
            CLASSIC  candy pink
            BLUSH    hotter pink
            HOG      dusty rose
            ZOMBIE   undead green-gray body, green sclera + red pupils
        - blink animation (intensity-modulated by mood -100 to +100)
        - ear wiggle, nose sniff on idle events (no booger pixels)
        - cute jump: tunable hop (default 400ms / 8px; games can raise it)
        - attack hop: 3-5 sequential pounce hops for handshake captures
        - wave ripple system: INCOMING / OUTGOING radio feedback
        - thunder flash: inverts avatar colors during storms
        - fruit tree: grows, sways, drops fruit, collapses empty
        - directional facing + walk transitions + attack shake

    GRASS (classic fat-blade turf, seasonal colors):
        - dense procedural blades (classic geometry everyone liked)
        - two-layer draw: most blades behind the pig, tall blades
          in front of the feet so leaves cross the hooves
        - palette follows SEASON (summer green / spring bright /
          autumn gold / winter frost)

    ANIM TEST (SETTINGS → ANIM TEST):
        - when ON, IDLE hotkeys [-] / [=] cycle emotions + FX
        - toast shows name: "ANIM 3/23  EXCITED"
        - mood system does not stomp demos while lab is on

    NIGHT MODE (20:00-06:00 local / SKY setting):
        stars, moon, RTC or synthetic cycle.
        SETTINGS → SKY: AUTO / DAY / NIGHT.


----[ 3.3 - WEATHER + SEASONS

    living weather cycle (mood biases odds; does not freeze "happy = clear"):
        CLEAR → CLOUDY → RAIN → STORM → back to life

    SEASON (SETTINGS → SEASON):
        AUTO     cycle SPRING → SUMMER → AUTUMN → WINTER every ~15 min
        SPRING   rain + real thunderstorms (lightning)
        SUMMER   gentle summer rain only (no bolts)
        AUTUMN   rain + falling leaves (denser when wet)
        WINTER   snow instead of rain + snow banks (сугробы)
                 banks grow while snowing, slowly melt when clear
                 denser flake count (72) for proper blizzard vibes

    CLOUDS   = drawn BEHIND the tree/pig (z-order fixed).
               fluffy puffs, parallax with grass, grow/shrink.
    RAIN     = seasonal streak colors; summer lighter, autumn duller.
    SNOW     = soft white flakes + ground drifts.
    LEAVES   = autumn-only particles.
    THUNDER  = spring storms invert colors via Avatar::thunderFlash().
    WIND     = gust particles when not precipitating.

    why? someone said "what if the pig had weather?" at 2am.
    then someone said "what if weather had seasons?" at a later 2am.
    nobody stopped us. particles rendered. the barn filed paperwork.
...
----[ 7.1 - MAIN MENU (from IDLE)
    ...
    RANK:
        ...
        FRUITRUN     - chill fruit collect in the real idle world ([G])
    ...
    SYSTEM:
        SETTINGS     - SKY, PIG SKIN, SEASON, ANIM TEST, brightness,
                       sound, dim, G0, boot mode, callsign,
                       WiFi / GPS / BLE / JANUS HOG / SD log
        BOARBROS     - whitelist management (BSSID + SSID)
        FILES        - SD + SPIFFS browser / delete
...
----[ 8.1 - FROM IDLE
    ...
    [M] MICPORK           mic dance (pig is the meter)
    [E] EVILPIG           SoftAP captive portal (lab)
    [P] PIGPASS           WPA2 password cracker
    ...
    ANIM TEST (if SETTINGS → ANIM TEST = ON):
        [-] previous face / FX demo
        [=] next face / FX demo
        toast names the animation. mood will not override demos.
...
----[ 8.3 - GLOBAL
    ...
    [0]  screenshot (saves .bmp to /m5porkchop/screenshots/)
    [P]  PigPass (password crack, IDLE hotkey)
...
----[ 10.2 - FROM RELEASE (recommended)
    ...
    One Pork fan binary (this tree, v0.1.8c (1.6)):
...
--[ 14 - RECENT PORK (v0.1.8c community pass)


----[ 14.1 - WHAT WE SHIPPED THIS PASS

    AVATAR / LOOKS:
        - unified cute chibi mesh; skins CLASSIC / BLUSH / HOG / ZOMBIE
        - zombie eyes green + red (not plain white voids)
        - emotion variety (happy squint, soft blink, sniff, jump faces)
        - classic fat-blade grass restored as the baseline turf
        - grass drawn in two layers (behind pig + leaves in front of feet)
        - ANIM TEST lab: SETTINGS toggle, IDLE [-]/[=] cycle demos

    SEASONS + WEATHER:
        - SETTINGS → SEASON: AUTO (15 min) or lock SPRING/SUMMER/AUTUMN/WINTER
        - spring: rain + thunder   summer: soft rain only
        - autumn: rain + falling leaves
        - winter: snow (dense flakes) + growing snow banks
        - clouds sorted behind tree/pig (no more tree-behind-clouds)

    FRUIT RUN ([G]):
        - same IDLE scene, same pig, same scale, seasons still apply
        - free walk left/right, jump, catch fruit dropped from growing trees
        - no obstacles, no game over — pure harvest vibes

    TOOLING / QOL (also in this era of the tree):
        - FILES browser (SD + SPIFFS)
        - PigPass resume / mask polish
        - EvilPig / PiggyBlues clean exits
        - boot patch notes toast (shorter, useful)

    (if something broke: open an issue. the pig already knows.)


----[ 14.2 - BRIEF OVERVIEW (ENGLISH)

    M5PORKCHOP is an open WiFi/BLE research firmware for the M5Cardputer:
    dual-band hunting (with optional ESP32-C5), wardriving, spectrum,
    BLE tools, and a living avatar world with mood, weather, seasons,
    and a small fruit-collect mini-game. This community pass focused on
    making the pig look and feel better (faces, skins, grass, seasons)
    and on playful quality-of-life features without rewriting the core
    RF stack. Build with PlatformIO (env m5cardputer). Attack only
    networks you own or have permission to test.


----[ 14.3 - THANK YOU TO THE CREATOR

    enormous thanks to **0ct0** — author of M5PORKCHOP / the original
    barn architect.

        https://buymeacoffee.com/0ct0

    the dual-band fangs, the mood machine, the Phrack-energy README,
    the horse that is the barn, the finite-state pig with better sleep
    hygiene than all of us: that foundation is yours.

    this section is just more straw on a structure you already framed.
    we ship features. you shipped a world.

    if you are reading this because you forked, starred, built, or
    just oinked at the screen: go thank the creator first.
    then break the firmware in interesting, legal ways.
...
--[ 15 - ONE PORK v0.1.8c (1.6) (FAN PACKAGING)

    VERSION: **v0.1.8c (1.6)**
        - 0.1.8c   = last known upstream M5PORKCHOP line this tree is based on
        - (1.6)    = pWnCrack (pwncrack.org) + 1.4.x tree + radio kludge (1.3 handoff + auto-OINK)
        - Notes:   releases/RELEASE_NOTES_1.6.md  (bugs + workarounds)
