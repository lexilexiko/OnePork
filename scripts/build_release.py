...
def find_pio():
    """Locate PlatformIO CLI (pio / platformio) on PATH or common install paths."""
    ...
    log_err("PlatformIO CLI (pio) not found. Install PlatformIO or add it to PATH.")
    sys.exit(1)
...
def run_cmd(cmd, description):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if result.stdout:
                # PlatformIO often prints the real error to stdout
                log_err(result.stdout[-4000:] if len(result.stdout) > 4000 else result.stdout)
            sys.exit(1)
...
def main():
    ...
    # Step 1: Build with PlatformIO (release env only)
    pio = find_pio()
    log_info(f"Using PlatformIO: {pio}")
    ...
    run_cmd(f'"{pio}" run -t clean -e m5cardputer', "Cleaning build artifacts")
    run_cmd(f'"{pio}" run -e m5cardputer', "Compiling release (m5cardputer)")
